...
from .labeledlayer.labeledlayer_pluginlayer import LabeledPluginLayer

from .labeledlayer.labeledlayer_pluginlayer_type import LabeledLayerPluginLayerType
...
class EasyCustomLabeling(QtCore.QObject):
    ...
    def __init__(self, iface):
        ...
        #***********************************************************************
        QtCore.QObject.__init__(self) #init from  QObject parent
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'EasyCustomLabeling_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&EasyCustomLabeling')
        ...
        for toolbar1 in toolbars:
            if toolbar1.windowTitle() == u'mLabelToolBar':
                self.toolbar = toolbar1
                test = False
                break
        if test:
            self.toolbar = self.iface.addToolBar(u'mLabelToolBar')
            self.toolbar.setObjectName(u'mLabelToolBar')
    ...
    def tr(self, message):
        ...
        return QtCore.QCoreApplication.translate('EasyCustomLabeling', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/EasyCustomLabeling/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'EasyCustomLabeling'),
            callback=self.run,
    ...
    def run(self):
        ...
        self.lbllayer.append(LabeledPluginLayer() )     #add selafin to list otherwise it can not work with multiple selafin files
        success = self.lbllayer[-1].loadLabeledPluginLayer(activelayer)
        
        if success:
            try:    #qgis 2
                qgis.core.QgsMapLayerRegistry.instance().addMapLayer(self.lbllayer[-1])
    ...
    def addToRegistry(self):
        ...
        """
        #qgis.core.QgsPluginLayerRegistry.instance().pluginLayerTypes()
        ...
        if u'labeledlayer' in reg.pluginLayerTypes():
            reg.removePluginLayerType('labeledlayer')
        self.pluginLayerType = LabeledLayerPluginLayerType()
